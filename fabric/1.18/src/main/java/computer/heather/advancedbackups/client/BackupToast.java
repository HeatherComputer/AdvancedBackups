    ...
    public Visibility draw(MatrixStack matrix, ToastManager manager, long startTime) {
        ...
        if (!exists) {
            if (title.equals(I18n.translate("advancedbackups.backup_finished"))){
                ...
                DrawableHelper.fill(matrix, 3, 28, 156, 29, ColourHelper.colour
        ...
        DrawableHelper.fill(matrix, 4, 28, Math.max(3, (int) f), 29, ColourHelper.colour
            (255, ClientConfigManager.progressBarRed.get().intValue(), ClientConfigManager.progressBarGreen.get().intValue(), ClientConfigManager.progressBarBlue.get().intValue()));
