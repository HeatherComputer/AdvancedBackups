    ...
    public static void register(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        dispatcher.register(ClientCommandManager.literal("backup").requires((runner) -> {
         ...
         .then(ClientCommandManager.argument("name", StringArgumentType.greedyString()).executes((runner) -> {
            ...
            Acknowledgment acknowledgment = MinecraftClient.getInstance().player.networkHandler.consumeAcknowledgment();
            MinecraftClient.getInstance().player.networkHandler.sendPacket(new CommandExecutionC2SPacket("backup snapshot " + snapshotName, Instant.now(), 0L, ArgumentSignatureDataMap.EMPTY, false, acknowledgment));
            return 1;
         })))
